...
def sample_diff_idx(log_diff_counts, sorted_diffs, epsilon, sensitivity):
    ...
    """
    return racing_sample(
        log_diff_counts - (epsilon * np.floor(sorted_diffs) / (2 * sensitivity))
    )
...
def sample_max_expo_distribution(expo_lambda, log_num_trials):
    ...
    results = (
        -np.log(-np.expm1(np.multiply(inverse_num_trials, log_uniform_draws)))
        / expo_lambda
    )
...
def sample_diff_idx_via_pnf(log_diff_counts, sorted_diffs, epsilon, sensitivity):
...
def sequence_from_diff(
    diff, diff_row, diff_col, diff_matrix, sampler=lambda ary: np.random.choice(ary, 1)
):
    ...
    ts = [np.searchsorted(diff_matrix[row, :], diff, side="right") for row in range(k)]
    for row in range(k):
        if row != diff_row:
...
def joint(item_counts, k, epsilon, neighbor_type, sample_diff_idx_func=sample_diff_idx):
    ...
    diff_idx = sample_diff_idx_func(
        log_diff_counts, sorted_diffs, epsilon, neighbor_type
    )
    diff_row, diff_col = (
        diffs_to_positions[0][diff_idx],
        diffs_to_positions[1][diff_idx],
    )
