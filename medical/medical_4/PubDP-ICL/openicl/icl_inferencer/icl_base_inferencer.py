...
class BaseInferencer:
    ...
    def __init_model(self, model_name, model_config, model_parallel, device_map, no_split_module_classes):
        ...
        if not model_parallel:
            ...
            if model_config is None:
                model_config = AutoConfig.from_pretrained(model_name, cache_dir="/scratch/gpfs/tw6664/llama/")
            ...
            self.model = AutoModelForCausalLM.from_pretrained(model_name, device_map=device_map,
                                                              ...
                                                              torch_dtype=torch.float16, cache_dir="/scratch/gpfs/tw6664/llama/")

    def __get_hf_model_from_name(self, model_name):
        if 't5' in model_name:
            return T5ForConditionalGeneration.from_pretrained(model_name, cache_dir="/scratch/gpfs/tw6664/llama/")
        elif "Llama-2" in model_name:
            return AutoModelForCausalLM.from_pretrained(model_name, cache_dir="/scratch/gpfs/tw6664/llama/")
        elif "llama" in model_name:
            return LlamaForCausalLM.from_pretrained(model_name, cache_dir="/scratch/gpfs/tw6664/llama/")
        else:
            return AutoModelForCausalLM.from_pretrained(model_name, cache_dir="/scratch/gpfs/tw6664/llama/")
    ...
    def __init_tokenizer(self, tokenizer_name):
        print("#"*10, tokenizer_name)
        if self.api_name == 'opt-175b':
            self.tokenizer = GPT2Tokenizer.from_pretrained("facebook/opt-30b", use_fast=False, cache_dir="/scratch/gpfs/tw6664/llama/")
        elif "Llama-2" in tokenizer_name:
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, cache_dir="/scratch/gpfs/tw6664/llama/")
        elif "llama" in tokenizer_name:
            self.tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name, cache_dir="/scratch/gpfs/tw6664/llama/")
        else:
...
class GenInferencerOutputHandler:
    ...
    def write_now(self, output_json_filepath: str, output_json_filename: str):
        self.results_dict = {
            str(idx): {
                'origin_prompt': self.origin_prompt_dict[str(idx)],
                'output': self.output_dict[str(idx)],
                'prediction': self.prediction_dict[str(idx)]
            } for idx in self.output_dict.keys()
        }
        with open(f'{output_json_filepath}/now{output_json_filename}.json', 'w', encoding='utf-8') as json_file:
