...
class FairBatchSampler(Sampler):
    def __init__(self, dataset, batch_size, target_ratios, replacement=True):
        ...
        # Extract sensitive values and group indices as tensors
        self.sensitive_values = torch.tensor([dataset.get_sensitive(idx) for idx in range(len(dataset))], device=self.device)
        self.class_indices = {
            label: (self.sensitive_values == label).nonzero(as_tuple=True)[0]
            for label in torch.unique(self.sensitive_values)
        }

        # Preload iterators or shuffle indices
        self.preloaded_indices = self._preload_indices()

    def _preload_indices(self):
        preloaded_indices = dict()
        for label, indices in self.class_indices.items():
            if self.replacement:
                # Infinite cycle of indices for replacement
                preloaded_indices[int(label.item())] = cycle(indices.tolist())
            else:
                # Shuffle indices for non-replacement
                shuffled_indices = indices[torch.randperm(len(indices), device=self.device)]
                preloaded_indices[int(label.item())] = iter(shuffled_indices.tolist())
    ...
    def __iter__(self):
        ...
        for _ in range(num_batches):
            batch = []
            for label, ratio in self.target_ratios.items():
                num_samples = int(self.batch_size * ratio)
                for _ in range(num_samples):
                    try:
                        # Fetch the next sample from preloaded indices
                        batch.append(next(self.preloaded_indices[label]))
                    except StopIteration:
                        # Reshuffle indices if replacement=False
                        if not self.replacement:
                            indices = self.class_indices[label]
                            shuffled_indices = indices[torch.randperm(len(indices), device=self.device)]
                            self.preloaded_indices[label] = iter(shuffled_indices.tolist())
                            batch.append(next(self.preloaded_indices[label]))
            yield batch
...
def get_dataset(args, split):
    ...
    if split == "train":
        target_ratios = {0: 0.5, 1: 0.5}  
        sampler = FairBatchSampler(dataset=data, batch_size=args.batch_size, target_ratios=target_ratios)
        # sampler = None
