...
with tf.Session() as sess:
    # load ckpt
    saver.restore(sess, './fair_ckpt/ML-1M_last_rmse_fair_0.001.ckpt')
    cur_feed = {R:test_r, male_user_idx: user['M'], female_user_idx: user['F'], male_item_idx: item['M'], female_item_idx: item['F']}
    pre = sess.run(pred_f, feed_dict=cur_feed)
    cur_dee = get_dee(pre.T)
    print("test dee:", cur_dee)
