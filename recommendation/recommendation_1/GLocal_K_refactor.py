...
adam_p = tf.train.AdamOptimizer(learning_rate=LR)
with tf.variable_scope('opt', reuse=tf.AUTO_REUSE) as scope:
    # Your code for defining variables
    optimizer_p = adam_p.minimize(loss_p)
...
with tf.variable_scope('opt', reuse=tf.AUTO_REUSE) as scope:
    # Your code for defining variables
    optimizer_f = adam_f.minimize(loss_f)
    if not scope.reuse:
...
with tf.Session() as sess:
    ...
    for i in range(epoch_p):
        if f_criterion is not None:
            cur_feed = {R: train_r, male_user_idx: user['M'], female_user_idx: user['F'], male_item_idx: item['M'], female_item_idx: item['F']}
        else:
            cur_feed = {R: train_r}
        tic = time()
        sess.run(optimizer_p, feed_dict=cur_feed)
        #optimizer_p.minimize(sess, feed_dict=cur_feed)
        pre = sess.run(pred_p, feed_dict=cur_feed)
        cur_other_loss = sess.run(loss_p_tmp, feed_dict=cur_feed)
        cur_floss = sess.run(fair_lossp, feed_dict=cur_feed)
        print("pre shape:", pre.shape)
        print("pre:", pre)
        print("cur_floss:", cur_floss)
        print("cur_other_loss:", cur_other_loss)

        t = time() - tic
        time_cumulative += t
        
        cur_dee = get_dee(pre.T)
    ...
    for i in range(epoch_f):
        ...
        if f_criterion is not None:
            ...
            cur_feed = {R: train_r}
        #optimizer_f.minimize(sess, feed_dict=cur_feed)
        sess.run(optimizer_f, feed_dict=cur_feed)
        ...
        if test_rmse < best_rmse and cur_dee < 0.07:
            best_rmse = test_rmse
            best_rmse_ep = i+1
    ...
    save_path = './model/' + dataset + f'_last_rmse_fair_{LR}.ckpt'
    np.save(f'./seeds/{seed}/' + dataset + f'_last_rmse_pred_fair_{LR}.npy', pre)
    np.save(f'./seeds/{seed}/' + dataset + f'_last_rmse_pred_vm_{LR}.npy', test_m)
    np.save(f'./seeds/{seed}/' + dataset + f'_last_rmse_pred_vr_{LR}.npy', test_r)
