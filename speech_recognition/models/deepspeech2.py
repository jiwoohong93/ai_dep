...
class DeepSpeech2(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.hidden_size = self.config.model.rnn_hidden_size # 768
        self.hidden_layers = self.config.model.nb_layers # 5
        self.rnn_type = eval(self.config.model.rnn_type) # nn.LSTM
        self.audio_conf = self.config.feature
        self.context = self.config.model.context # 20
        ...
        rnn_input_size = self.config.feature.input_dim
        ...
        self.ctc_loss = nn.CTCLoss(blank=self.PAD_token, reduction='none', zero_infinity=True)


    def forward(self, x, lengths, trns, logit=False):
        ...
        # valid_idx = (~torch.isinf(loss)) & (~torch.isnan(loss))
        if logit:
